...
namespace tic_tac_toe
{
    public class GameState
    {
        ...
        private bool AreSquearesMarked((int, int)[] squares, Player player)
        {
        ...
        private bool DidMoveWin(int r, int c, out WinInfo winInfo)
        {
            ...
            if (AreSquearesMarked(row, CurrentPlayer))
            {
                winInfo = new WinInfo { Type = WinType.Row, Number = r };
                return true;
            }

            if (AreSquearesMarked(col, CurrentPlayer))
            {
                winInfo = new WinInfo { Type = WinType.Column, Number = c };
                return true;
            }

            if (AreSquearesMarked(mainDiag, CurrentPlayer))
            {
                winInfo = new WinInfo { Type = WinType.MainDiagonal };
                return true;
            }

            if (AreSquearesMarked(antiDiag, CurrentPlayer))
            {
